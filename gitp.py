...
if __name__ == "__main__":
    ...
    for key in range(len(string.ascii_letters + string.digits + " !?.") + 1):
        ...
        print(f"{key}: {result}")  # Вывод всех ключей и соответствующих сообщений
        
        # Условие: если результат выглядит осмысленно, например, содержит часть ожидаемого текста,
        # можно вручную добавить проверку. Здесь же просто добавим последний результат:
        if " " in result:  # Пример простой проверки, можно добавить более точное условие
            final_key = key
